    ...
    Sub Data_Save()
        ...
        For Each gr In Me.tabA.Rows
            If gr.RowType = DataControlRowType.DataRow Then
                L1 = gr.FindControl("LanguageID")
                sSql = "Update Languages Set "
                T1 = gr.FindControl("RptEmailSubject")
                sSql = sSql & " RptEmailSubject=N'" & Replace(T1.Text, "'", "''") & "'"

                T1 = gr.FindControl("RptEmailMessage")
                sSql = sSql & ", RptEmailMessage=N'" & Replace(T1.Text, "'", "''") & "'"


                sSql = sSql & " where"
                sSql = sSql & " LanguageID=" & L1.Text
                Trace.Warn(sSql)
                CF.Runquery(sSql)
            End If
        Next
