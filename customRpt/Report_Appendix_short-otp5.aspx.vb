...
Partial Class CustomRpt_Report_HPI
    ...
    'populate_mvpi
    Protected Sub populate_mvpi(sender As Object, e As System.Web.UI.WebControls.GridViewRowEventArgs)
        If e.Row.RowType = DataControlRowType.Header Then Exit Sub
        If e.Row.RowType = DataControlRowType.Footer Then Exit Sub
        Response.Write("DDDDDDDDDDDDDDDDDDDDDDDDDDDDDd")
        Response.End()
    ...
    Protected Sub Page_LoadComplete(sender As Object, e As EventArgs) Handles Me.LoadComplete
        Dim i As Decimal, t As HtmlTable, td As HtmlTableCell, tr As HtmlTableRow, lb As Label
        Dim image As Image
        Dim sql As String
        Dim v As Integer
        Dim conn As SqlConnection, cmd As SqlCommand, rs As SqlDataReader

        'Response.Write(Request.QueryString("CandidateID"))
        'Response.End()
        sql = "SELECT DISTINCT valid FROM candidates WHERE CandidateID=" & Request.QueryString("CandidateID")
        conn = New SqlConnection(ConfigurationManager.ConnectionStrings("cMain").ConnectionString)
        conn.Open()
